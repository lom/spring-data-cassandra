...
public final class CassandraSimpleTypeHolder extends SimpleTypeHolder {
    ...
    static {
		...
		final CodecRegistry codecRegistry = CodecRegistry.DEFAULT_INSTANCE;

		final Map<Class<?>, Class<?>> primitiveWrappers = new HashMap<>(8);
	...
	private static Map<Class<?>, DataType> classToDataType(CodecRegistry codecRegistry,
		...
		final Map<Class<?>, DataType> classToDataType = new HashMap<>(16);

		DataType.allPrimitiveTypes().forEach(dataType -> {

			final Class<?> javaType = codecRegistry.codecFor(dataType).getJavaType().getRawType();
	...
	private static Map<Name, DataType> nameToDataType() {
		final Map<Name, DataType> nameToDataType = new HashMap<>(16);

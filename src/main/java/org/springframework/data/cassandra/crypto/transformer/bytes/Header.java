 ...
 * prependend to each encrypted columnCryptoState, and itself is not encrypted. Header
 * format is the following:
 * <ul>
 * <li>byte 0..2: "magic" number identifying the format as Cassandra-crypto
 * encrypted sequence.
...
public class Header {
    ...
    // "CC1" is a "magic number" identifying Cassandra-Crypto version 1
    ...
    public static Header create(String keyName, boolean compressed, boolean withHMAC) {
        ...
        try {
            ...
            throw new CassandraCryptoException("Can't encode in " + KEY_NAME_CHARSET, e);
        }

        if (keyNameBytes.length > KEY_NAME_MAX_SIZE) {
            throw new CassandraCryptoException("Key name '" + keyName
    ...
    public String getKeyName() {
        ...
        try {
            ...
            throw new CassandraCryptoException("Can't decode with " + KEY_NAME_CHARSET, e);
        }
    }
}
